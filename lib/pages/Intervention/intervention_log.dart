...
class InterventionLog extends StatefulWidget {
  @override
  _InterventionLogState createState() => _InterventionLogState();
...
class _InterventionLogState extends State<InterventionLog> {
  List interventionA = [
    'To help you relax, try doing a breathing exercise. ',
    'Remember, when you are upset, identifying the trigger is the first step toward feeling better. ',
    'Imagine you are someone else dealing with the same stressor you are: what would you tell them? ',
    'When upset about something ask yourself: what am I telling myself about the event that is making me upset? Is there an alternative thought that would be more useful? Is there evidence to contradict your negative thought? ',
    'Try to find time to use the progressive muscle relaxation techniques you learned tonight. '
  ];

  @override
  Widget build(BuildContext context) {
    final InterventionArgs? args = context.getRouteArguments();
    final questions = args?.interventionQuestions?.interventionLogQuestions ?? [];
                    ...
                    itemBuilder: (_, index) => Padding(
                          ...
                          child: QuestionWidget(
                            ...
                            onAnswerChanged: (answer) {
                              final index = args?.interventionLogAnswer?.indexWhere(
                                      (element) => element.questionID == answer?.questionID) ??
                                  -1;
                              if (index >= 0 && answer != null) {
                                args?.interventionLogAnswer?[index] = answer;
                              }
                              setState(() {});
                            },
                          ),
              ...
              TextButton(
                style: ButtonStyle(
                    backgroundColor: MaterialStateProperty.all(
                        _validateAnswers(args?.interventionLogAnswer)
                ...
                onPressed: () {
                  if (_validateAnswers(args?.interventionLogAnswer)) {
